...
async def test_permission_grant_emits_lease_and_resumes(
    ...
    request = await _drain_until(client, lambda e: e.type == "permission.request", timeout=2.0)
...
async def test_permission_deny_fails_job(
    ...
    final = await _drain_until(client, lambda e: e.type == "job.failed", timeout=2.0)
...
async def test_lease_refresh_extends_expiry(
    ...
    await client.send(grant)
    completion = await _drain_until(client, lambda e: e.type == "lease.granted", timeout=2.0)
...
def test_lease_manager_unit_tests() -> None:
    ...
    leases = LeaseManager()
    lease = leases.grant(permission="x", resource=None, operation=None, seconds=60)
