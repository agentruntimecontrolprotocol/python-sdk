...
await client.request(client.envelope(EXT_TUNE, payload={...}))
await client.request(client.envelope(EXT_GAIN, payload={...}))
capture = await client.request(client.envelope(EXT_CAPTURE, payload={...}))
audio   = await client.request(client.envelope(EXT_DEMODULATE,
...
- `main.py` — client side: tune, gain, capture, demodulate, then
  one optional unknown for §21.3.
- `config.py` — endpoint, defaults, extension names.
- `extensions.py` — payload models + registry.
- `runtime_handlers.py` — runtime-side dispatcher with the
  optional-vs-required nack logic. Hardware calls stubbed.
...
uses `extensions["optional"] = True` as the marker. See
