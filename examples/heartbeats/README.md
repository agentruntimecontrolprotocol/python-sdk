...
async def _heartbeat_reaper(self) -> None:
    while True:
        ...
        for w in self.roster.stale(deadline_seconds=DEADLINE_S):
            if (job := w.in_flight_job) is not None:
                task = self._jobs_to_tasks.pop(job, None)
                if task is not None:
                    await self._dispatch(task)   # same idempotency_key
            self.roster.remove(w.worker_id)
```
...
- `main.py` — boots supervisor + small worker pool in-process.
- `config.py` — heartbeat constants, identity helpers.
- `supervisor.py` — dispatch + reaper. `_dispatch` is the one to read.
- `worker.py` — accept/started/heartbeat/terminal lifecycle. The
  actual work is `do_work(...)` — stubbed.
- `roster.py` — capability index + freshness bookkeeping.
