...
async for env in client.events():
    if env.type == "human.input.request":
        asyncio.create_task(
            fan_out(client, request=env, destinations=DESTINATIONS)
        )

# inside fan_out:
done, _ = await asyncio.wait(tasks, timeout=timeout,
                             return_when=asyncio.FIRST_COMPLETED)
responded_by, value = next(iter(done)).result()
await client.send(client.envelope(
...
- `main.py` — opens session, dispatches each inbound HITL request.
- `config.py` — endpoint + destinations.
- `relay.py` — `fan_out` is the file. First-wins resolution,
  loser-channel cancellation, deadline handling.
- `channels.py` — per-destination adapters; stubbed.
