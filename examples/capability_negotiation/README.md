...
```python
profiles[peer.name] = from_capabilities(
    peer.name, c.negotiated_capabilities,
)        # cost / latency / class come back at session open

chain = candidate_chain(profiles, constraints)
reply = await invoke_with_fallback(
    clients=clients, chain=chain,
    tool="chat.completion", arguments={...}, trace_id=trace_id,
)
```
...
- `main.py` — open all peers, route one request, print rollup.
- `config.py` — peers + fallback chains + ceilings.
- `extensions.py` — typed view of marketplace capability fields.
- `router.py` — eligibility filter + chain construction.
- `fallback.py` — ordered handoff with retry classification.
- `metering.py` — tenant rollup from `metric` envelopes.
