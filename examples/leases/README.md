...
await client.request(
    client.envelope("permission.request", payload={
        "permission": "host.write",
        "resource":  f"host:{host}/usr/bin/systemctl/api-gateway",
        "operation": "restart",
...
- `main.py` — opens session, wires the agent, runs one ticket.
- `config.py` — endpoint, lease horizons, allowlists.
- `agent.py` — emits `kind: thought`, calls into the sandbox. The
  LLM call is stubbed (`llm_loop`) so the file is about ARCP, not
  Anthropic.
- `leases.py` — `LeaseCache.acquire` issues `permission.request`
  and returns the materialized `Lease`.
- `sandbox.py` — argv classifier; demands a live lease for every
  call.
