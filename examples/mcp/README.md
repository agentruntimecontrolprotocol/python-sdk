...
async with stdio_client(upstream_params()) as (read, write), \
    ...
    extensions = await advertise_from_mcp(mcp)   # MCP tool list
    async for envelope in inbound:
...
- `main.py` — the bridge. `handle_invoke` is the file. Runtime
  wiring (transport, session manager) is symmetric with
  `arcp.runtime.server.ARCPRuntime` and elided.
- `upstream.py` — MCP server invocation (`StdioServerParameters`).
